...
func deal_with_damage():
	if player_inattack_zone and Global.player_current_attack == true:
		if can_take_damage == true:
			...
			if health <= 0:
				print("skeleton has been killed")
				experience = experience + 25
				Global.experience = Global.experience + experience
				$AnimatedSprite2D.play("death") #this should work but i need to wait or someting
				#await $AnimatedSprite2D.animation_finished this works but another anim is playing...
				print("Sk_experience gained: ", experience)
				print("Sk_global xp: ", Global.experience)
...
func reset(): #call after player health = 0   IN PROGRESS KIND OF WORKS FOR NOW
	if player_health <= 0:                    
